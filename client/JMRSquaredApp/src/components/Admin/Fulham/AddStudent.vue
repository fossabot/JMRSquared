        ...
        methods: {
            ...
            canGoForward() {
                if (this.currentPage == 0) {
                    let answer = this.tenantUserName.length > 2 && this.tenantName.length > 2 && this.tenantNumbers.length == 10 && this.tenantName.split(" ").length > 1;
                    this.txtError = this.tenantName.split(" ").length > 1 ? "" : "Please provide a surname and name separated by a space under full name";
                    if (this.txtError.length < 2) {
                        this.txtError = answer ? "" : "All fields are required";
                    }
                    return answer;
                } else if (this.currentPage == 1) {
                    this.txtError = this.leaseTotalMonths > 0 ? "" : "Please provide a start date that is before the end date";
                    if (this.txtError.length < 2) {
                        this.txtError = this.tenantRoom.length > 0 ? "" : "Please provide a valid room number";
                    }
                    return this.leaseTotalMonths >= 0 && this.tenantRoom.length > 0
                } else if (this.currentPage == 2) {
                    let answer = !isNaN(this.rentAmount) && this.rentAmount > 0 && (!this.hasDeposit || (this.hasDeposit && !isNaN(this.depositAmount) && this.depositAmount > 0));
                    this.txtError = answer ? "" : "All fields are required";
                    return answer;
                } else if (this.currentPage == 3) {
                    let answer = (!this.hasBusary || this.hasBusary && this.busaryProvider.length > 2) && this.nextOfKin.name.length > 2 && this.nextOfKin.contact.length > 2 && this.nextOfKin.relationship.length > 2;
                    this.txtError = answer ? "" : "All fields are required";
                    return answer;
                } else {
                    return false;
