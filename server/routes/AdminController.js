...
router.get('/all', function(req, res) {
    Admin.find().populate(['documents']).populate(['notifications']).then((admins) => {
...
router.get('/GetById/:adminID', function(req, res) {
    ...
    Admin.findById(adminID).populate(['documents']).populate(['notifications']).then((admin) => {
        if (admin == null) {
            res.status(500);
...
router.post('/login', function(req, res) {
    if(req.body.useEmail){
        Admin.findOne({
            ...
            pass: req.body.pass
        }).populate(['documents']).populate(['notifications']).then((admin) => {
            if (admin == null) {
                res.status(500);
